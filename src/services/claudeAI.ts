...
const CREDIBLE_SOURCE_ONLY_DIRECTIVE = `CRITICAL SOURCE RULE — ALLOW-LIST, NOT A BAN-LIST: Do not name or cite any source unless it is one of these credible types: government bodies and regulators (e.g. SEC, Census Bureau, Eurostat, ministries, central banks), official industry/trade associations, multilateral/intergovernmental organizations (e.g. IMF, World Bank, WTO), company filings/investor relations/press releases/earnings releases, reputable news outlets (e.g. Reuters, Bloomberg, WSJ, FT), and top-tier consulting/professional services firms (e.g. McKinsey, BCG, Bain, PwC, KPMG, Deloitte, EY, Accenture). Everything else is FORBIDDEN as a named source — this is an allow-list, so when in doubt, leave the source unnamed.
HEURISTIC FOR DETECTING FORBIDDEN SOURCES: Syndicated market-sizing research firms are NOT credible even if not explicitly named below — as a rule of thumb, do not cite any firm whose name contains words like "Research", "Market(s)", "Insights", "Intelligence" (in a market-data-vendor sense), "Analytics", or "Data" as a business-name suffix (e.g. Emergen Research, The Business Research Company, Research Nester, Fact.MR, Coherent Market Insights, Straits Research, Custom Market Insights, Zion Market Research, Allied Market Research, Polaris Market Research, Reports and Data, Future Market Insights, Research and Markets, MarketsandMarkets, GlobalData, Fortune Business Insights, Mordor Intelligence, Precedence Research, IMARC, Transparency Market Research, Technavio, Grand View Research, or any similarly-named publisher). None of these — named here or not — may ever appear as a cited source, in any field, including "citations", "source", table cells, or inline text.
If you cannot trace a figure to one of the credible source types in the allow-list, present it as your own analytical estimate without naming any source at all — never invent, guess, or default to a market-research-firm-sounding name to fill the gap.`;
...
- NEVER cite MarketsandMarkets, GlobalData, Fortune Business Insights, Mordor Intelligence, Precedence Research, IMARC Group, Research and Markets, Transparency Market Research, Technavio, Grand View Research, or similar syndicated market-research publishers as a source. If a figure traces back to one of these, present it as your own analytical estimate instead, or attribute it to a primary source.
...
export const SECTION_DEFINITIONS_V2: Record<string, { title: string; tableHint: string; chartHint: string; subsectionHint: string }> = {
  ...
  market_innovation: {
    ...
    tableHint: 'Return a "tables" array with ONE table titled "Innovation Tracker". Headers: ["Innovation / Initiative", "Innovator (Startup / Company)", "Category", "Description", "Stage"]. Category values: "Product Innovation", "Process Innovation", "Business Model Innovation", "Patent / IP", or "R&D Breakthrough". Stage values: "Concept", "Pilot", "Launched", "Scaling", or "Granted" (for patents). Description: 2-3 detailed sentences on what is novel about it, who is behind it, and why it matters for the industry. Include 8-12 rows, with STRONG PRIORITY on startup activity specifically within the target industry and target geography/region (search for regional startup ecosystems, accelerators, and recently funded startups in this exact market) — supplement with notable patents/R&D from established players. Prioritise the last 12-18 months.',
    chartHint: 'No chart needed. Set chartSpec to null.',
    subsectionHint: 'bodyParagraphs[0]: 2-3 sentences summarising the innovation landscape — which categories of innovation are most active, notable startups to watch in this specific industry and region, and the overall pace of patent activity. No subsections.',
  },
...
- Every point within one chart MUST be the same metric, measured on the same basis, varying only along the x-axis (for example: all points are "percent of large enterprises scaling AI agents", varying by year). Never combine two different statistics into a single series — a figure for "percent of applications integrated" and a figure for "percent of software shipping with agentic AI" are different metrics and belong in different charts, or in neither, no matter how naturally their dates line up. A series that changes meaning between points is worse than no chart.
