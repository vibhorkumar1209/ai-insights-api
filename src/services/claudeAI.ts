...
- Every point within one chart MUST be the same metric, measured on the same basis, varying only along the x-axis (for example: all points are "percent of large enterprises scaling AI agents", varying by year). Never combine two different statistics into a single series — "40% of applications integrated by 2026" and "33% of software shipping with agentic AI by 2028" are different metrics and belong in different charts, or in neither. A series that changes meaning between points is worse than no chart.
