...
router.post('/', aiLimiter, (req: Request, res: Response) => {
  ...
  const input: ObjectionHandlingInput = {
...
// ── GET /api/objection-handling/:jobId ───────────────────────────────────────

router.get('/:jobId', (req: Request, res: Response) => {
  const job = getObjectionHandlingJob(req.params.jobId);
  if (!job) { res.status(404).json({ error: 'Job not found' }); return; }
  res.json(job);
});

// ── GET /api/objection-handling/:jobId/stream (SSE) ──────────────────────────
